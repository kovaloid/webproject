...
<fmt:message bundle="${loc}" key="local.data.cars.ready_radio" var="ready_radio"/>
<fmt:message bundle="${loc}" key="local.data.cars.not_ready_radio" var="not_ready_radio"/>
        ...
        <div class="panel-body">
            <div class="alert alert-success">
                <form action="main" method="post" class="navbar-form" role="form">
                    ...
                    <select title="${choose_color}" required name="color" class="form-control margin">
                        <option selected disabled>${choose_color}</option>
                        <c:forEach var="color" items="${sessionScope.colors_list}">
                            <option value="${color}">${color}</option>
                        </c:forEach>
                    </select>
                    <select title="${choose_mark}" required name="brand" class="form-control margin">
                        <option selected disabled>${choose_mark}</option>
                        <c:forEach var="car" items="${sessionScope.cars_list}">
                            <option value="${car}">${car}</option>
                        </c:forEach>
                    </select>
                    <select title="${choose_driver}" required name="driver_id" class="form-control margin">
                        <option selected disabled>${choose_driver}</option>
                        <mytag:select_box bean="${sessionScope.drivers_list}"/>
                    </select>
                    <br/>
                    <label><input name="ready" type="radio" value="да" checked> ${ready_radio}</label><br>
                    <label><input name="ready" type="radio" value="нет"> ${not_ready_radio}</label><br/>
