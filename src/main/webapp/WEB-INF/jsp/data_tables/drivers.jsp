...
<jsp:include page="/WEB-INF/jsp/blocks/login_block.jsp">
    <jsp:param name="page" value="${pageContext.request.contextPath}/main?command=drivers"/>
</jsp:include>
...
<div class="main">
    <div class="well">
        ...
        <c:if test="${sessionScope.drivers_tables_amount ne 0}">
            <c:if test="${sessionScope.drivers_tables_amount ne 1}">
                <div align="center">
                    <ul class="pagination">
                        <c:forEach var="i" begin="1" end="${sessionScope.drivers_tables_amount}">
                            <c:if test="${sessionScope.drivers_table_number eq i}">
                                <li class="active"><a
                                        href="${pageContext.request.contextPath}/main?command=drivers&number=${i}">${i}</a>
                                </li>
                            </c:if>
                            <c:if test="${sessionScope.drivers_table_number ne i}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/main?command=drivers&number=${i}">${i}</a>
                                </li>
                            </c:if>
                        </c:forEach>
                    </ul>
